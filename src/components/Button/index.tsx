...
export function Button({ children, icon, rtl = false, ...props }: ButtonProps) {
  ...
  return (
    <button
      ...
      } before:clip-path before:absolute before:-bottom-[4px] before:right-0 before:left-0 before:h-1 before:bg-black before:disabled:bg-bw-600 md:before:disabled:-bottom-[4px] md:before:-bottom-[5px]`}
      {...props}
    >
      {children}
